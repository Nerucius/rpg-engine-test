...
public class Room implements Disposable {
    ...
    private void createEvents() {
        ...
        MapLayer objLayer = tileMap.getLayers().get("events");
        ...
        for (MapObject obj : objLayer.getObjects()) {
            ...
            if (type.equalsIgnoreCase("spawn")) {
                this.spawn = new Vector2(x + w / 2, y + h / 2);
                continue;
            }
    ...
    private void createLights() {
        ...
        MapLayer objLayer = tileMap.getLayers().get("lights");
    ...
    public Vector2 getSpawn() {
        if (this.spawn == null)
            throw new Error("Room has no spawn: " + this.tmxFile.toString());

        // Return global coordinate spawn.
        return tmp.set(this.spawn).add(position.x, position.y);
    }

    public void setPosition(float x, float y) {
        // TODO implement ability to move rooms (and everything inside) around.
    }
    ...
    public OrthogonalTiledMapRenderer getTiler() {
        return tiler;
    }
}
